...
def navigate(place, cur_dir: Dir):
    if (place == ".."):
...
def read_dirs(lines, cur_dir: Dir) -> list[Dir]:
    dirs: list[Dir] = []
    for line in lines:
        line = line.split()
...
for line in lines:
    if (isLS and is_command(line)):
        isLS = False
        dirs = read_dirs(lsCommands, cur_dir)
        lsCommands = []
    elif (isLS):
    ...
    if (is_command(line)):
        ...
        if (command_type == CMD.CD):
            place_to_navigate = cd_loc(line)
            new_dir = navigate(place_to_navigate, cur_dir)
            cur_dir = new_dir
...
def size(cur_dir: Dir):
    if cur_dir.is_file():
        return cur_dir.size
    for child in cur_dir.children:
        cur_dir.size += size(child)
    return cur_dir.size
...
sum = 0
for dir in all_dirs:
    if (dir.size <= 100000 and not dir.is_file()):
        sum += dir.size
